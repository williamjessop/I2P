...
class Lesson extends React.Component {
  ...
  updateProgress() {
    ...
    axios
      .post(`${urlBase}/lesson/updateProgress`, body, {
  ...
  listParser(content, listType, answer) {
    ...
    let listItems = listArray.map((item, i) => {
      if (i === answer - 1) {
        ...
        return (
          <div key={i}>
            <li style={{ width: "100%", textAlign: "left", margin: "2.5px" }}>
              {item}
            </li>
            <br />
          </div>
    ...
    return (
      <ul
        className="lessonContentNumberedList"
        style={{ overflow: "initial", listStyleType: listType }}
      >
        {listItems}
      </ul>
    );
        ...
        <div className="lessonContent">
          ...
          {this.state.pages[this.state.currentPage].map((block, index) => {
            if (block.contentType === "text") {
              return (
                <p key={index} className="lessonContentText">
                  {block.content}
                </p>
              );
              ...
              return (
                <p
                  key={index}
                  className="lessonContentSubText"
                  style={{ float: block.align, fontStyle: "italic" }}
                >
                  {block.content}
                </p>
              );
              ...
              return (
                <p
                  key={index}
                  className="lessonContentLeftAlign"
              ...
              return (
                <div
                  key={index}
                  style={{ width: "500px", margin: "auto", paddingTop: "20px" }}
                >
                  <Row>
                    <h5 style={{ float: "left", textAlign: "left" }}>
                      {block.listTitle}
                    </h5>
                  </Row>
                  <Row>
                    {this.listParser(block.content, "disc", block.correct)}
                  </Row>
                  <br />
                </div>
              );
              ...
              return (
                ...
                >
                  ...
                  <Row>
                    {this.listParser(block.content, "decimal", block.correct)}
                  </Row>
                  <br />
                </div>
              );
              ...
              return (
                <h1 key={index} className="lessonContentHeader">
                  {block.content}
                </h1>
              );
            } else if (block.contentType === "subHeader") {
              return (
                <h5 key={index} className="lessonContentSubHeader">
                  {block.content}
                </h5>
              );
            } else if (block.contentType === "bold") {
              return (
                <p key={index} className="lessonContentBold">
                  <b>{block.content}</b>
                </p>
              );
              ...
              return <p key={index}>There was an issue with the database. Please contact the developer.</p>;
            }
          })}
          <br />
          <br />
